...
files_list <- list.files(path = "data/processed", pattern = "constant_list", full.names = TRUE)
...
read_and_label <- function(file_path) {
  ...
  label <- sub("data/processed/(.*)_constant_list.csv", "\\1", file_path)
...
#Save----
write_csv(coverage_long, "data/processed/coverage_long_constant.csv")
write_csv(coverage_wide, "data/processed/coverage_wide_constant.csv")
