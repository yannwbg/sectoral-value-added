...
#Load data----
coverage_long <- read_csv("data/processed/coverage_long_current.csv") %>%
  select(-country)

coverage_wide <- read_csv("data/processed/coverage_wide_current.csv")

un4 <- read_csv("data/processed/un4_current.csv") %>%
  select(iso3, year, sector = name, currency, value, starts_with("growth"))

un3 <- read_csv("data/processed/un3_current.csv") %>%
  select(iso3, year, sector = name, currency, value, starts_with("growth"))

oecd <- read_csv("data/processed/oecd_current.csv") %>%
  mutate(value = value*1000000) %>% #Unit is always 6
  select(iso3, year, sector = name, currency = currency_label, value, starts_with("growth"))

national <- read_csv("data/processed/national_current.csv") %>%
  select(iso3, year, sector, currency, value, starts_with("growth"))

un4_gap_filler <- read_csv("data/temp/un4_current_gap_filler_data.csv") %>% 
...
un3_gap_filler <- read_csv("data/temp/un3_current_gap_filler_data.csv") %>% 
  filter(!(is.na(growth_to_next) & is.na(growth_to_prev))) %>%
  group_by(iso3, year) %>%
  mutate(keep = (series_code == max(series_code))) %>%
  filter(keep == TRUE) %>%
...
data_sourced <- data_compiled %>%
  left_join(best_source %>%
            ...
            by = "iso3")
#Save----
write_csv(data_sourced, "data/temp/current_compiled.csv")
