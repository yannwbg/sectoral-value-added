...
#Load original data
files_list <- list.files(path = "data/raw/global", pattern = "4_constant_full", full.names = TRUE)

raw <- map(files_list, function(x) {
  
  read_csv(x,
           col_types = c("cccccnncncnnc")) %>%
...
write_csv(data, "data/temp/un4_constant_full.csv")
write_csv(footnote, "data/temp/un4_constant_footnote_full.csv")
